 ...
 *
 * Versione integrata: mantiene tutte le funzioni esistenti (snake_case,
 * fetchNewsByHash, BookmarkButton, sezione Continua a leggere) e aggiunge
 * breadcrumb, CTA torna al feed/home per orientamento mobile.
 */
import { notFound } from 'next/navigation';
import Image from 'next/image';
import Link from 'next/link';
import { ExternalLink, ArrowLeft, FileText, Home as HomeIcon, ChevronLeft } from 'lucide-react';
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    ...
    // Open Graph per anteprime su condivisioni social
    openGraph: {
      title: news.title,
      description: news.description ?? undefined,
    ...
    <>
      ...
      <main className="mx-auto max-w-[900px] px-4 pb-24 pt-6 sm:px-6 sm:pb-12 bg-[#080808]">
        ...
        <nav className="mb-4 flex items-center gap-2 text-xs">
          ...
          <Link
            href="/news"
            className="text-zinc-500 transition hover:text-[#e8c800]"
          >
            News
          </Link>
          <span className="text-zinc-700">/</span>
          <span
            className="truncate uppercase tracking-widest text-zinc-300"
        ...
        {/* Link indietro (mantiene il tuo originale) */}
        <Link
          href="/news"
        ...
        <article className="rounded-3xl border border-[#1f1f1f] bg-[#0d0d0d] p-5 sm:p-8">
          ...
          <h1
            className="text-2xl uppercase leading-tight tracking-tight text-white sm:text-4xl font-black mb-6"
            style={{ fontFamily: 'var(--font-archivo-black)' }}
          >
            {news.title}
          </h1>
        ...
        {/* CTA torna al feed + home (extra mobile) */}
        <div className="mt-10 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Link
            href="/news"
            className="flex items-center justify-center gap-2 rounded-xl border border-[#1f1f1f] bg-[#0d0d0d] py-3 text-xs uppercase tracking-widest text-zinc-400 transition hover:border-[#e8c800]/40 hover:text-[#e8c800]"
            style={{ fontFamily: 'var(--font-dm-mono)' }}
          >
