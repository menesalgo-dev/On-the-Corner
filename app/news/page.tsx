...
interface PageProps {
  searchParams: Promise<{ category?: string; source?: string; page?: string; }>;
}
...
export default async function NewsPage({ searchParams }: PageProps) {
  ...
  const [newsData, bookmarkHashes] = await Promise.all([
    getNewsItems({ category: currentCategory, source: currentSource, page: currentPage, limit: 24 }),
    ...
    <div className="flex min-h-screen bg-[#080808] text-white">
      ...
      <main className="flex-1 p-6 md:p-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 border-b border-zinc-800 pb-4">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tight text-[#e8c800]" style={{ fontFamily: 'var(--font-archivo-black)' }}>Notizie</h1>
            <p className="text-sm text-zinc-400 mt-1">Feed aggregato da 21 fonti premium con dedup intelligente.</p>
          </div>
          <div className="text-xs font-mono tracking-widest text-zinc-500 uppercase mt-2 md:mt-0" style={{ fontFamily: 'var(--font-dm-mono)' }}>
        ...
        {formattedNews.length === 0 ? (
          <div className="mt-12 flex justify-center">
            <EmptyState title="Nessuna Notizia" description="Nessun risultato trovato per i filtri selezionati." />
          </div>
        ) : (
